...
func doMinerPing(addr string) []byte {
	cmd := `docker exec --interactive $(docker ps --filter name=miner --format "{{.ID}}") miner peer ping ` + addr + ` 2>&1`
...
func doApiRequest(address string) []byte {
	...
	//url := "https://helium-api.stakejoy.com/v1/hotspots/" //redirects to default API
...
func main() {
	...
	fmt.Println("PISCES VERSION (docker only)")
	fmt.Println();

	//t, err := tail.TailFile("/mnt/data/docker/volumes/*_miner-log/_data/console.log", tail.Config{Follow: true})
	t, err := tail.TailFile("/home/pi/hnt/miner/log/console.log", tail.Config{ReOpen: true, MustExist: false, Follow: true})
